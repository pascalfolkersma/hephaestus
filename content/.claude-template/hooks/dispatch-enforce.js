...
// ===========================================================================
// ADAPTER CONSTANTS — Claude Code target (ADR 0039 §2–§3, M12.9)
//
...
//   core/lib/target-adapter.js → CLAUDE_CODE_ADAPTER
//
// Surfaces resolved here (Decision 0036 §3):
//   1. deny convention       : exit 2 (Claude) — never exit 0 + payload
//   2. session-id source     : stdin `session_id` (snake_case)
//   3. tool-name vocabulary  : Bash / Edit / Write / Agent / Task
//   4. state root            : .claude/
//   5. flow-context dir      : .claude/flows/
//   6. inline-ok path        : .claude/flows/<sessionId>/inline-ok
//   7. agent-type source     : stdin `agent_type` / env CLAUDE_AGENT_TYPE
// ===========================================================================
const ADAPTER = Object.freeze({
  // Tool name vocabulary (Claude Code).
  SHELL_TOOL:      'Bash',
  EDIT_TOOL:       'Edit',
  CREATE_TOOL:     'Write',
  DISPATCH_TOOLS:  new Set(['Agent', 'Task']),
  ...
  // Dispatch-enforce sidecar config path (derived from state root; mirrors
  // resolveDispatchEnforceConfigPath('claude-code', ...) in core/lib/target-adapter.js).
  DISPATCH_ENFORCE_CONFIG: '.claude/dispatch-enforce.config.json',

  // Deny convention: Claude Code uses exit 2 for graceful per-tool deny.
  // DO NOT change to exit 0 — that is the Copilot convention (ADR 0039 §3).
  DENY_EXIT_CODE:  2,

  // Agent-type: sourced from stdin field then env-var fallbacks.
  // No side-channel file needed (Claude Code has native agent_type).
  AGENT_TYPE_FIELD: 'agent_type',
});
// ===========================================================================
...
//
// Tool names use ADAPTER constants — no hardcoded 'Bash'/'Edit'/'Write' below.
...
function loadSourcePathRules() {
  const configPath = path.join(process.cwd(), ADAPTER.DISPATCH_ENFORCE_CONFIG);
  let raw;
  try {
  ...
  for (const entry of config.sourcePaths) {
    ...
    SPECIALIST_RULES.push({ tool: ADAPTER.EDIT_TOOL,   pattern: pat, agents });
    SPECIALIST_RULES.push({ tool: ADAPTER.CREATE_TOOL, pattern: pat, agents });
  }
}
...
function patchGitAgentRules() {
  ...
  for (const rule of SPECIALIST_RULES) {
    if (rule.tool !== ADAPTER.SHELL_TOOL) continue;
...
function evaluateFlowGate(toolName, toolInput, sessionId) {
  try {
    // Only intercept dispatch-tool calls (from adapter: Agent, Task).
...
function evaluateBashBypassGate(toolInput, agentType) {
  // Only inspect Bash tool calls (caller already knows toolName === ADAPTER.SHELL_TOOL,
...
function testPattern(rule, toolInput) {
  ...
  if (rule.tool === ADAPTER.EDIT_TOOL || rule.tool === ADAPTER.CREATE_TOOL) {
 ...
 *
 * Claude Code convention: exit 2 (per ADR 0039 §3 / ADAPTER.DENY_EXIT_CODE).
...
async function main() {
  ...
  if (stdinText) {
    ...
    if (parsed && typeof parsed === 'object') {
      ...
      // Agent-type field from adapter (ADAPTER.AGENT_TYPE_FIELD = 'agent_type').
      agentType =
        parsed[ADAPTER.AGENT_TYPE_FIELD] ||
        parsed.agent_id ||
      ...
      // Session-id field from adapter (ADAPTER.SESSION_ID_FIELD = 'session_id').
      sessionId = parsed[ADAPTER.SESSION_ID_FIELD] || '';
  ...
  // Resolve from ADAPTER.FLOWS_DIR + session_id + /inline-ok.
  ...
  if (toolName === ADAPTER.SHELL_TOOL) {
